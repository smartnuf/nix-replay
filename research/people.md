# Initial people and credit map

This file begins with verifiable publication roles. It is deliberately not a
biographical narrative or a ranking of contribution. Publication authorship,
software authorship, maintenance, governance, documentation, support, and
community-building are different kinds of credit.

| Person or group | Verified role in this slice | Sources |
| --- | --- | --- |
| Eelco Dolstra | Author of the 2006 thesis; co-author of the 2004 Nix and 2008/2010 NixOS papers | S0001-S0004, S0007 |
| Merijn de Jonge | Co-author of the 2004 LISA Nix paper | S0001, S0004 |
| Eelco Visser | Co-author of the 2004 LISA Nix paper and 2008 Nix Build Farm paper | S0001, S0004, S0009 |
| Armijn Hemel | Co-author of the 2007 system-configuration paper | S0004, S0008 |
| Andres Löh | Co-author of the 2008 and 2010 NixOS papers | S0003, S0004 |
| Nicolas Pierron | Co-author of the 2010 journal NixOS paper | S0004 |
| Nix contributors | Collectively credited by current Nix citation metadata | S0007 |
| NixOS contributors | Responsible group named for the current NixOS manual | S0006 |

## Editorial safeguards

- Say "the paper credits" when the evidence is a byline.
- Do not equate the earliest paper found with sole invention.
- Do not infer a person's present role from an old publication.
- Seek consent and current self-description before portraying living people in
  an episode or using interview material.
- Expand this map through repository history, governance records, archived
  discussions, and interviews; do not let academic publication alone define
  whose work counts.

## Open work

The next history pass should establish institutional context, project and
release milestones, the growth of Nixpkgs and NixOS, governance transitions,
and representative contributors across research, engineering, maintenance,
documentation, and community work.
